...
import { listPullIn, type PullInRow } from "@/lib/queries/transactions";
...
import { PullInStatusButtons } from "./status-buttons";

export const metadata: Metadata = { title: "Pull-In" };
...
export default async function PullInPage({
  searchParams,
}: {
  ...
  const rows = await listPullIn({ branchId: scope, status });

  const columns: Column<PullInRow>[] = [
    {
      key: "item",
      header: "Item",
      render: (row) => (
    ...
    },
    {
      ...
      render: (row) =>
        row.status === "pending" ? (
          <PullInStatusButtons id={Number(row.id)} />
        ) : (
  ...
  return (
    <>
      <PageHeader
        title="Pull-In"
        description="Stock arriving at a branch, pending receipt into inventory."
        action={
          <Link
            href="/pull-in/new"
        ...
        {STATUS_FILTERS.map((filter) => {
          ...
          return (
            <Link
              ...
              href={filter.value ? `/pull-in?status=${filter.value}` : "/pull-in"}
              className={cn(
      ...
      <FadeIn>
        <Card>
          <DataTable
            ...
            rows={rows}
            getKey={(row) => row.id}
            emptyTitle="No pull-in requests"
            emptyDescription="Stock deliveries will show up here once created."
          />
        </Card>
      </FadeIn>
    </>
